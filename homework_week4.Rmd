...
```
#Result of plot 6. People who spent more time getting acute terapy are more likely to spent more time getting post acute therapy. 
...
```{r}
ggplot(data = hf_summary, aes(x = age, y = acute_therapy, colour = sex)) +
  geom_point()+
   facet_wrap (~ fracture_type)
```
```{r}
ggplot(data = hf_summary, aes(x = age, y = comorbidity, colour = sex)) +
  geom_smooth(method = "lm")+
   facet_wrap (~ fracture_type)
...
```{r}
ggplot(data = hf_summary, aes(x = age, y = management_method, colour = sex)) +
 geom_point(size = 0.3, alpha = 0.5, position =  position_jitter(0.1))+
facet_wrap (~ in_hospital_mortality)
```

```{r}
ggplot(data = hf_summary, aes(management_method, fill = sex)) +
 geom_bar()+
facet_wrap (~ status_12m)
```

```{r}
ggplot(data = hf_summary, aes(x = age, y = days_to_operation, colour = sex)) +
 geom_smooth(method = "lm" )
```


```{r}
ggplot(data = hf_summary, aes(days_to_operation, fill = fracture_type)) +
 geom_histogram()
```